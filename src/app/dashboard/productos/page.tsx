...
function StatCard({ icon, value, label, sub, iconColor }: {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-4">
      ...
      <p className="text-xs text-slate-400 font-mono mt-0.5">{sub}</p>
    </div>
  );
}
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-8 pr-3 py-1.5 border border-slate-200 rounded-lg text-xs text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-green-500 w-48"
                />
              </div>
              <select
                value={catFilter} onChange={(e) => setCatFilter(e.target.value)}
                className="border border-slate-200 rounded-lg text-xs text-slate-600 px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-green-500 bg-white"
              >
              ...
              {filteredProductos.map((prod) => {
                ...
                return (
                  <tr key={prod.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    <td className="py-3 pr-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400 truncate max-w-[180px]">{prod.descripcion}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-3 pr-3">
                    ...
                    <td className="py-3 pr-3">
                      {prod.genera_ticket && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-medium bg-purple-50 text-purple-600 border border-purple-100">
      ...
      {!isLoading && activeTab === 'categorias' && (
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {categorias.length === 0 && (
              <p className="col-span-2 text-xs text-slate-400 text-center py-8">No hay categorías registradas.</p>
            )}
          </div>
        </div>
      )}
